...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var help = function (weight) {
      return A4($Graphics$Element.container,
      500,
      100,
      $Graphics$Element.topLeft,
      $Graphics$Element.leftAligned(A2($Basics._op["++"],
      $Text.fromString("Press up arrow to increase weight,\nand down arrow to decrease.\nCurrent weight is "),
      $Text.fromString($Basics.toString(weight)))));
   };
   var center = F2(function (_p0,p) {
      var _p1 = _p0;
      return _U.update(p,
      {position: {ctor: "_Tuple2",_0: $Basics.fst(p.position) - $Basics.toFloat(_p1._0) / 2,_1: $Basics.toFloat(_p1._1) / 2 - $Basics.snd(p.position)}});
   });
   var centerForm = A2($Graphics$Collage.filled,$Color.blue,A2($Graphics$Collage.rect,4,4));
   var particleForm = function (p) {
      return A2($Graphics$Collage.move,p.position,A2($Graphics$Collage.filled,$Color.red,$Graphics$Collage.circle(p.weight)));
   };
   var massForm = function (_p2) {
      var _p3 = _p2;
      var _p5 = _p3._1;
      var _p4 = _p3._0;
      return $Graphics$Collage.group(_U.list([A2($Graphics$Collage.move,
                                             {ctor: "_Tuple2",_0: _p4,_1: _p5},
                                             A2($Graphics$Collage.filled,$Color.gray,$Graphics$Collage.circle(10)))
                                             ,A2($Graphics$Collage.move,
                                             {ctor: "_Tuple2",_0: _p4 + 20,_1: _p5 + 20},
                                             $Graphics$Collage.text($Text.fromString("centre of mass")))]));
   };
   var momentForm = function (_p6) {
      var _p7 = _p6;
      var _p9 = _p7._1;
      var _p8 = _p7._0;
      return $Graphics$Collage.group(_U.list([A2($Graphics$Collage.move,
                                             {ctor: "_Tuple2",_0: _p8,_1: _p9},
                                             A2($Graphics$Collage.filled,$Color.black,$Graphics$Collage.circle(10)))
                                             ,A2($Graphics$Collage.move,
                                             {ctor: "_Tuple2",_0: _p8 + 10,_1: _p9 - 15},
                                             $Graphics$Collage.text($Text.fromString("moment")))]));
   };
   var moment = function (particles) {
      var f = F2(function (p,_p10) {
         var _p11 = _p10;
         return {ctor: "_Tuple2",_0: _p11._0 + p.weight * $Basics.fst(p.position),_1: _p11._1 + p.weight * $Basics.snd(p.position)};
      });
      return A3($List.foldr,f,{ctor: "_Tuple2",_0: 0.0,_1: 0.0},particles);
   };
   var massCenter = function (particles) {
      var mass = $List.sum(A2($List.map,function (_) {    return _.weight;},particles));
      var _p12 = moment(particles);
      var x = _p12._0;
      var y = _p12._1;
      return {ctor: "_Tuple2",_0: x / mass,_1: y / mass};
   };
   var particleworld = function (ps) {
      return A2($List._op["::"],
      massForm(massCenter(ps)),
      A2($List._op["::"],centerForm,A2($List._op["::"],momentForm(moment(ps)),A2($List.map,particleForm,ps))));
   };
   var canvas = F2(function (_p13,particles) {    var _p14 = _p13;return A3($Graphics$Collage.collage,_p14._0,_p14._1,particleworld(particles));});
   var world = F3(function (weight,dimensions,particles) {    return $Graphics$Element.layers(_U.list([help(weight),A2(canvas,dimensions,particles)]));});
   var Particle = F2(function (a,b) {    return {weight: a,position: b};});
   var clamp = F2(function (_p15,value) {    var _p16 = _p15;return A2($Basics.max,A2($Basics.min,value,_p16._1),_p16._0);});
   var weight = function () {
      var f = F2(function (prev,input) {    return A2(clamp,{ctor: "_Tuple2",_0: 1,_1: 40},prev + input);});
      return A3($Signal.foldp,f,1,A2($Signal.map,function (_) {    return _.y;},$Keyboard.arrows));
   }();
   var particle = function () {
      var f = F2(function (w,p) {    return {weight: w,position: p};});
      return A3($Signal.map2,
      f,
      A2($Signal.map,$Basics.toFloat,weight),
      A2($Signal.map,
      function (_p17) {
         var _p18 = _p17;
         return {ctor: "_Tuple2",_0: $Basics.toFloat(_p18._0),_1: $Basics.toFloat(_p18._1)};
      },
      $Mouse.position));
   }();
   var centredParticle = A3($Signal.map2,center,$Window.dimensions,particle);
   var particles = A3($Signal.foldp,F2(function (x,y) {    return A2($List._op["::"],x,y);}),_U.list([]),A2($Signal.sampleOn,$Mouse.clicks,centredParticle));
   var main = A4($Signal.map3,world,weight,$Window.dimensions,particles);
   return _elm.Main.values = {_op: _op
                             ,clamp: clamp
                             ,weight: weight
                             ,Particle: Particle
                             ,particle: particle
                             ,centredParticle: centredParticle
                             ,particles: particles
                             ,moment: moment
